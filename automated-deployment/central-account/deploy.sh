...
usage () { echo "
    ...
    -a -- Central Datalake AWS Account ID
...
if ! $sflag
then
    S3_BUCKET=lakeformation-automation-$SOURCE_REGION-$SOURCE_ACCOUNT
fi
if ! $nflag
then
    STACK_NAME="lakeformation-automation-source"
fi
...
aws s3 cp $DIRNAME/output/packaged-template.yaml s3://$S3_BUCKET/${STACK_NAME}-packaged-template.yaml
PACKAGED_STACK_URL="https://${S3_BUCKET}.s3.${SOURCE_REGION}.amazonaws.com/${STACK_NAME}-packaged-template.yaml"
...
if ! aws cloudformation describe-stacks --profile $PROFILE --stack-name $STACK_NAME; then
  ...
  aws cloudformation create-stack \
    ...
    --template-url $PACKAGED_STACK_URL \
